...
// This defines all possible permissions in the application
const permissions = [
...
const planPermissions: { [key: string]: string[] } = {
  ...
  "Business Pro": [
    'dashboard.view', 'contract.create', 'contract.view.all',
    'risk_report.view', 'user.read', 'user.invite', 'user.delete',
    'user.update.role', 'role.read', 'role.create', 'role.update', 'role.delete'
  ],
  Enterprise: [
    'dashboard.view', 'contract.create', 'contract.view.all',
    'risk_report.view', 'user.read', 'user.invite', 'user.delete',
...
const plans = [
  {
    ...
    isFree: true,
    features: {
      list: ["AI Risk Engine (5 checks/month)", "Smart Contracts (2/month)", "Basic Buyer Verification"],
      bestFor: "Ideal for individuals and freelancers starting out.",
      limits: { riskChecks: 5, contracts: 2 }
    }
  },
  {
    name: "Starter",
    priceInr: 1191100,
    razorpayPlanId: "plan_RFag5aeyGUt6Dj",
    isFree: false,
    features: {
      list: ["AI Risk Engine (200 checks/Year)", "Smart Contracts (20/Year)", "Basic Ecommerce Store (limited products)", "Buyer Verification (Basic)", "Community Support"],
      bestFor: "Best for freelancers, consultants, micro-businesses"
    }
  },
  {
    name: "Growth",
    priceInr: 2392300,
    razorpayPlanId: "plan_RFagQlJRwPfJQJ",
    isFree: false,
    features: {
      list: ["AI Risk Engine (700 checks/Year)", "Smart Contracts (75/Year)", "Advanced Ecommerce Store", "Centralized Buyer Blocklist Access", "Basic ERP Modules (Accounting + Inventory)", "Verified Buyers & Suppliers Network (Standard)", "Email & Chat Support"],
      bestFor: "Best for small & mid-size MSMEs"
    }
  },
  {
    name: "Business Pro",
    priceInr: 3493400,
    razorpayPlanId: "plan_RFagmPiUchK3FP",
    isFree: false,
    features: {
      list: ["AI Risk Engine (1,500 checks/Year)", "Smart Contracts (200/Year)", "Full Ecommerce Suite", "Advanced ERP Modules (Inventory, Accounting, Payroll, Taxation)", "Verified Buyers & Suppliers Network (Premium)", "Third-Party Integrations (GST, MCA, Tally, Zoho, Busy, Vyapar)", "Financial & Legal Assistance (Basic)", "Priority Support"],
      bestFor: "Best for growing enterprises & multi-location businesses"
    }
  },
  {
    name: "Enterprise",
    priceInr: 4994900,
    razorpayPlanId: "plan_RFahCAzuDb6R3s",
    isFree: false,
    features: {
      list: ["Unlimited AI Risk Checks", "Unlimited Smart Contracts", "Complete Ecommerce + ERP Suite", "Buyer/Supplier Network (Enterprise Tier)", "Advanced Financial & Legal Integrations", "API Access for Custom Development", "Government Schemes & Funding Advisory", "Dedicated Account Manager", "24/7 Priority Support"],
      bestFor: "Best for large corporations, associations & clusters"
    }
  }
];
...
async function main() {
  ...
  const adminOrg = await prisma.organization.upsert({
    where: { name: 'Solviser Admin' },
    update: {},
    create: { name: 'Solviser Admin' }
  });
  ...
  console.log(`✅ Super Admin ${adminUser.email} seeded`);
  ...
  await prisma.product.createMany({
    data: [
      ...
      },
      {
        ...
        createdById: adminUser.id,
        name: "Neem Wood Comb",
        description: "Natural neem wood comb",
        price: 15,
        stock: 0,
        category: "Sustainables",
      },
